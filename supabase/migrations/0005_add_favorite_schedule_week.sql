create index if not exists idx_class_schedules_program_week
  on class_schedules(program_id, weekday, start_time, valid_from desc);
...
as $$
  ...
  page as (
    select *
    from ranked
    where row_number <= greatest(least(coalesce(p_limit, 120), 200), 1)
  ),
  stats as (
    select
...
grant execute on function favorite_class_schedule_week(
  uuid[],
  text,
  integer,
  integer
